    ...
    render: (v: number, item: CashflowTransaction) => (
      <span
        className={`font-semibold ${
          item.type === "in" ? "text-emerald-700" : "text-rose-700"
        }`}
      >
        {item.type === "out" ? "– " : "+ "}
        {IDR(v)}
...
export default function CashflowPage() {
  ...
  // Derive endpoint with month filter
  ...
  useEffect(() => {
    fetchFromBackend("/api/cashflow/categories")
      .then((res) => {
        // Defensive: only set categories if the array items have name and type
        let arr: any[] = [];
        if (Array.isArray(res)) arr = res;
        else if (res && Array.isArray(res.data)) arr = res.data;
        // Check if all items are Category
        if (arr.every(
          (c) =>
            c && typeof c === "object" &&
            typeof c.id === "string" &&
  ...
  // ── Compute summary from loaded rows ──
  ...
  const handleEdit = (item: CashflowTransaction) => {
    setEditItem(item);
    setModalOpen(true);
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Manage all cash-in and cash-out transactions
          </p>
        </div>
