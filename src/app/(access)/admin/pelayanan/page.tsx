...
import { ChevronDown, ChevronUp, UserPlus, UserMinus, Search, Plus, Pencil, Trash2, X, Check } from 'lucide-react';
...
type DepartmentMember = {
  ...
  jemaat?: { nama_lengkap: string; gender: string | null; phone_number: string | null } | null;
};
...
export default function PelayananPage() {
  ...
  const loadAll = async () => {
    setLoading(true);
    await Promise.all([loadDepartments(), loadLeaderOptions(), loadAllJemaat()]);
    setLoading(false);
  };
  ...
  const loadDepartments = async () => {
    const { data } = await supabase
      .from('departments')
      .select('id, nama_pelayanan, deskripsi, leader_id, created_at, jemaat(nama_lengkap)')
  ...
  const loadLeaderOptions = async () => {
    const { data } = await supabase
      ...
      .order('nama_lengkap');
    setLeaderOptions((data ?? []).map((j: any) => ({ value: j.id, label: j.nama_lengkap })));
  };
  ...
  const loadMembers = async (deptId: string) => {
    const { data } = await supabase
      ...
      .select('id, jemaat_id, role_pelayanan, tanggal_join, jemaat(nama_lengkap, gender, phone_number)')
  ...
  const handleDeptSubmit = async () => {
    ...
    const { error } = editDept
      ? await supabase.from('departments').update(payload as any).eq('id', editDept.id)
  ...
  const handleAddMember = async (deptId: string, jemaatId: string) => {
    ...
    if (error) {
      ...
      setRoleInput(prev => { const n = { ...prev }; delete n[jemaatId]; return n; });
  ...
  const handleRemoveMember = async (deptId: string, memberId: string) => {
    if (!confirm('Yakin ingin mengeluarkan anggota ini dari pelayanan?')) return;
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 mb-1">Pelayanan</h1>
          <p className="text-sm text-gray-500">Kelola departemen pelayanan dan penugasan anggota</p>
        </div>
        <button
          onClick={openAddDept}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6 flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-gray-600">Nama Pelayanan *</label>
                <input
                  type="text"
                  value={deptName}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-gray-600">Leader</label>
                <select
                  value={deptLeader}
      ...
      {departments.length === 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-gray-500 mb-3">Belum ada pelayanan. Mulai dengan menambah pelayanan baru.</p>
          <button onClick={openAddDept} className="text-sm text-blue-600 hover:underline">
      ...
      <div className="flex flex-col gap-3">
        {departments.map(dept => {
          ...
          const filteredMembers = deptMembers.filter(m =>
            m.jemaat?.nama_lengkap.toLowerCase().includes(mSearch.toLowerCase())
          );
          ...
          return (
            <div key={dept.id} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
              ...
              <div className="flex items-center gap-4 px-5 py-4">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900">{dept.nama_pelayanan}</p>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 mr-2">
                    <span className="text-xs text-gray-500 bg-gray-100 px-2.5 py-1 rounded-full font-medium">
                      {isExpanded && members[dept.id] ? `${members[dept.id].length} anggota` : '···'}
                    </span>
                    {isExpanded
                ...
                <div className="border-t border-gray-100">
                  ...
                  <div className="flex gap-1 px-5 pt-4">
                    <div className="flex gap-1 bg-gray-100 p-1 rounded-lg w-fit">
                      <button
                        onClick={() => setActiveTab(prev => ({ ...prev, [dept.id]: 'members' }))}
                      ...
                      <button
                        onClick={() => setActiveTab(prev => ({ ...prev, [dept.id]: 'add' }))}
                      ...
                      <div>
                        {deptMembers.length > 4 && (
                          <div className="relative mb-3">
                            ...
                            <input
                              type="text"
                              value={mSearch}
                              onChange={e => setMemberSearch(prev => ({ ...prev, [dept.id]: e.target.value }))}
                        ...
                        {filteredMembers.length === 0 ? (
                          <div className="py-8 text-center text-xs text-gray-400">
                            {mSearch ? 'Anggota tidak ditemukan.' : 'Belum ada anggota di pelayanan ini.'}
                          </div>
                        ) : (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-xs text-gray-400">
                                    {m.role_pelayanan
                                      ? <span className="text-purple-600 font-medium">{m.role_pelayanan}</span>
                                      : 'Anggota'
                                    }
                                    ...
                                    {m.tanggal_join && (
                                      <span className="ml-1.5">
                                        Bergabung {new Date(m.tanggal_join).toLocaleDateString('id-ID', {
                      ...
                      <div>
                        <div className="relative mb-3">
                          ...
                          <input
                            type="text"
                            value={aSearch}
                            onChange={e => setAddSearch(prev => ({ ...prev, [dept.id]: e.target.value }))}
                        ...
                        {availableJemaat.length === 0 ? (
                          <div className="py-8 text-center text-xs text-gray-400">
                            {aSearch ? 'Jemaat tidak ditemukan.' : 'Semua jemaat sudah menjadi anggota pelayanan ini.'}
                          </div>
                        ) : (
                          <div className="flex flex-col gap-2">
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                  <input
                                    ...
                                    onChange={e => setRoleInput(prev => ({ ...prev, [j.id]: e.target.value }))}
                                  ...
                                  />
                                  <button
                                    onClick={() => handleAddMember(dept.id, j.id)}
